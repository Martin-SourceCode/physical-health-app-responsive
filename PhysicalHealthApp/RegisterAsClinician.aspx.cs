...
{
    public partial class RegisterAsClinician : System.Web.UI.Page
    {
        ...
        private void GetTnCs()
        {
            ...
            if (dt.Rows.Count > 0)
            {
                try { this.lblTermsAndConditions.Text = dt.Rows[0]["cliniciantermsandconditions"].ToString(); } catch { }
            }
        }
...

        {
            ...
            string sql = "INSERT INTO app_user(usertype, userpassword, gmccode, matchedorganisationid,  emailaddress, firstname, lastname, organisationid, isclinician, acceptedtermsandconditions)";
            sql += " VALUES (@usertype, crypt(@userpassword, gen_salt('bf', 8)), @gmccode, CAST(@matchedorganisationid AS INT), @emailaddress, @firstname, @lastname, CAST(@organisationid AS INT), true, CAST(@acceptedtermsandconditions AS BOOL))";
